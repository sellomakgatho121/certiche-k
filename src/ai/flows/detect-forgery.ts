...
const DetectForgeryInputSchema = z.object({
  ...
  documentType: z.string().optional().describe('The type of the document(s) as specified by the user, if any. This helps focus the analysis. If multiple document types are present, this might be a general category or one of the primary types. If "None / Let AI Identify" is chosen or left blank, the AI should attempt to identify the document type(s).')
});
export type DetectForgeryInput = z.infer<typeof DetectForgeryInputSchema>;

const DetectForgeryOutputSchema = z.object({
  isForged: z.boolean().describe('A boolean indicating whether any document in the image is highly likely to be a forgery *based on its content*. This should be true ONLY if specific, significant evidence of *content manipulation* is found on any document. It should NOT be true solely due to scan artifacts that DO NOT obscure content or poor image quality preventing *content verification*.'),
  anomalies: z
    .array(z.string())
    .describe('A detailed list of specific anomalies, inconsistencies, or signs of *content manipulation* detected in any document(s) within the image. Each anomaly should be clearly described. Differentiate between benign scan artifacts (which do not affect content integrity unless they *obscure content*) and actual suspicious findings related to *content*. If multiple documents are present, specify WHICH document the anomaly pertains to (e.g., "Altered text on the ID card", "Irregular signature on the Driver\'s License"). Note if quality issues or *obscuring scan artifacts* limit *content analysis* for any specific document.'),
  confidence: z
    .number()
    .min(0).max(1)
    .describe(
      'A confidence score (0.0 to 1.0) indicating the likelihood OF ANY DOCUMENT IN THE IMAGE BEING FORGED *based on its content*. A score closer to 1.0 suggests high probability of *content forgery* based on strong, direct evidence of *content manipulation* on at least one document. Common scan artifacts or poor image quality alone (without other indicators of *content tampering* OR if they DO NOT obscure content) should result in a LOW confidence score for forgery (e.g., 0.0-0.3). If *content analysis* is limited by quality or *obscuring artifacts*, confidence in any assessment will be low; reflect this by keeping forgery confidence low unless *content manipulation* is evident on a document.'
    ),
...
2.  **Content Forgery Detection Principles (Applied to Each Document):**
    ...
    *   **Identify Anomalies in Content (Link to Specific Document):** Detail every suspicious *content element* or inconsistency. For each anomaly, explain why it is indicative of potential *content forgery*, especially in the context of its identified document type. **Crucially, if multiple documents are present, specify WHICH document the *content anomaly* pertains to in the anomaly description string itself.** If an observation is likely a benign scan artifact that *does not alter or obscure critical content*, note it as such and clarify it does NOT, on its own, indicate *content forgery*. If an artifact *does* obscure content, note that verification of that content is hindered.
    *   **Assess Forgery Likelihood (\`isForged\` - Overall, Based on Content):** Based on concrete evidence of *content manipulation* on *any* document, determine if the submission \`isForged\`. \`isForged\` should ONLY be true if there are specific, identifiable signs of *content forgery* on at least one document. If critical *content areas* are unverifiable due to extremely poor image quality or **severe, obscuring scan artifacts** *without other direct evidence of content forgery on any document*, \`isForged\` should be \`false\`. **Base this decision on the presence of manipulated content. Benign scan artifacts that do not obscure content MUST NOT lead to \`isForged = true\`.**
    *   **Confidence Score (\`confidence\` - Overall, Based on Content):** Provide a \`confidence\` score (0.0 to 1.0) indicating the likelihood OF ANY DOCUMENT BEING FORGED *due to content manipulation*. A score closer to 1.0 suggests a high probability of *content forgery*, based on *strong, direct evidence of content manipulation on at least one document*. A score closer to 0.0 suggests no direct evidence of *content forgery* was found on any document. Common scan artifacts or poor image quality alone (especially if they DO NOT obscure critical content) should result in a LOW confidence score. If content analysis is severely limited by quality or *obscuring artifacts*, confidence in *any* assessment will be low; reflect this by keeping forgery confidence low unless *content manipulation* is clearly evident on a document.
...
3.  **Specific Considerations for Scanned Documents and Image Quality (Relative to Content Verification):**
    *   **Scanned Document Artifacts vs. Content Issues:** Distinguish common scanning artifacts (Moiré, skew, dust, resolution limits, uneven illumination) from deliberate *content manipulation*. **These artifacts, in isolation and IF THEY DO NOT OBSCURE CRITICAL CONTENT, are NOT signs of *content forgery*** and should not contribute to an \`isForged\` conclusion or a high \`confidence\` score if the underlying *content* is legible and appears intact. Focus on inconsistencies *within each document's content itself*. If a scan artifact *does* significantly obscure critical content, note it as a limitation to verification for that part of the document, which may lower overall confidence in authenticity but is not itself proof of forgery unless other content issues are present.
