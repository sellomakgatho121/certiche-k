...
const prompt = ai.definePrompt({
  ...
  1.  **Leverage Document Type:** The document has been identified as a \`{{{documentType}}}\`. You MUST use this information to tailor your analysis. Consider specific expected features, layouts, security elements (if applicable for this type like holograms, watermarks, microprinting), common content, typical paper quality or texture appearance, font styles, and known areas of scrutiny or potential manipulation for a \`{{{documentType}}}\`. For example:
      ...
      *   If "Other" is specified as the document type, or if the type is very generic, conduct a more general but still meticulous examination based on the visual evidence and any provided description, looking for universal signs of tampering.
  2.  **Scrutinize with Extreme Detail:** Assume that forgeries can be very subtle. Look for minute inconsistencies that might be overlooked by a standard review.
  3.  **Authenticity Assessment:** Based on your comprehensive analysis, determine if the document is likely authentic. Set \`isAuthentic\` to false if there are any medium or high severity anomalies indicative of forgery. If image quality severely hinders analysis to the point where critical features cannot be verified, set \`isAuthentic\` to false and clearly state in the summary that this is due to *inability to verify*, not a confirmed forgery.
  4.  **Anomaly Detection:** Identify and list ALL anomalies. For each anomaly, provide:
      ...
      *   \`description\`: A detailed explanation of what the anomaly is and why it raises suspicion. If it's a common scan artifact (see point 7), explain that it's likely benign.
  ...
  5.  **Consider Advanced Techniques:** Be aware of advanced forgery methods, including digital manipulation, pixel-level alterations, font recreation, subtle background tampering, and chemical alterations (though the latter is harder to detect from an image alone, look for tell-tale signs like discoloration).
  6.  **Cross-Verification (Conceptual):** Although you cannot access external databases, think about what elements would ideally be cross-verified (e.g., ID numbers, addresses). Mention if an anomaly pertains to such an element.

  Specific Considerations for Scanned Documents and Image Quality:
  7.  **Scanned Document Artifacts:** It is crucial to differentiate between common, benign scanning artifacts and actual signs of manipulation. Common artifacts include slight skew, dust specks, minor shadows from the scanner lid, moiré patterns, typical scan resolution limitations, and slightly uneven lighting. **These common artifacts, by themselves, are NOT indicators of forgery.** Only flag scan-related issues as anomalies if they are unusually pronounced, obscure critical information in a way that seems intentional, or are combined with other specific, content-based suspicious indicators. If an anomaly is purely a common scan artifact, categorize it as "Scan Artifact" with a 'low' severity and clearly state in the description that it is likely a benign characteristic of the scanning process.
  8.  **Multiple Documents in One Image:** If the uploaded image contains multiple distinct documents, attempt to analyze each document individually, referencing the overall \`{{{documentType}}}\` if it applies to all (e.g., "Set of Identification Documents") or focusing on primary document if it's a mix. Your report should clearly delineate findings for each document if separable. Anomalies should specify the document they pertain to. The overall \`isAuthentic\` status should reflect the assessment of all documents; if one is suspect due to forgery (not just scan quality), the overall authenticity may be questionable.
  9.  **Image Quality Impact:** If the image quality (e.g., poor lighting, unfavorable angle, obstructions, blurriness, low resolution, reflections) significantly hinders a thorough analysis, this MUST be explicitly stated in your summary and in the description of any related anomalies. Detail which specific areas are obscured or made difficult to assess. **Critically, do NOT conclude a document is inauthentic or forged *solely* because poor image quality prevents full verification of some features.** Instead, state that the authenticity of those specific features (or the entire document, if severely affected) cannot be fully confirmed due to these quality issues. The \`isAuthentic\` assessment should reflect this: if analysis is severely hindered, \`isAuthentic\` should be set to false due to the inability to verify, not because it is confirmed as forged. The summary must clearly articulate this distinction.
  10. **Summary:** Provide a concise summary of your findings, highlighting the most critical observations, your overall conclusion on the document's authenticity (considering the specific \`{{{documentType}}}\`), and explicitly mention any limitations due to scan quality, multiple documents, or image issues. If authenticity cannot be confirmed due to quality, state this clearly.
